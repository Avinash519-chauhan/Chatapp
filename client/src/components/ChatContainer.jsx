            ...
            {messages.map((msg,index)=>(
                ...
                "flex-row-reverse"}`}>
                    {msg.image ? (
                        ...
                        <p className={`p-2 max-w-[200px] md:text-sm dont-light rounded-lg mb-8 break-all bg-violet-500/30 
                        text-white ${msg.senderId === 'authUser._id' ? 'rounded-br-none' : 'rounded-bl-none'}`}>{msg.text}</p>
                    )}
                    <div className="text-center text-x5">
                        <img src={msg.senderId === authUser._id ? authUser._id ?.profilePic || assets.avatar_icon : selectedUser?.profilePic || 
